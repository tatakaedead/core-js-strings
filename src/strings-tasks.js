...
function sumOfCodes(str) {
  ...
  const newLengthStr = str.length;
  let sum = 0;
  for (let i = 0; newLengthStr > i; i += 1) {
...
function orderAlphabetically(str) {
  const alphabetStr = str.split('').sort();
  return alphabetStr.join('');
}
...
function isPalindrome(str) {
  ...
  const newStr = str.toLowerCase().replaceAll(' ', '').replaceAll(/[^\w]/g, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < newStr.length; i += 1) {
    const maxLength = newStr[i];
    if (maxLength.length > result.length) {
      result = maxLength;
    }
  }
  return result;
...
function extractNameFromTemplate(value) {
  const newStr = value.slice(0, -1).split(' ');
...
function extractEmails(str) {
  const strReplace = str.replaceAll(';', ' ');
  const strToArray = strReplace.split(' ');
  return strToArray;
}
...
function getCardId(value) {
  ...
  const newValue = deck.indexOf(value);
  return newValue;
}
